<?				/* ---------------- USB-VM-GRAPH.PHP V1.7 For RMS-100 (C) 2016 ETHERTEK CIRCUITS ---------------- */
...
$vm_info = array( "name" => array(), "mode" => array(), "shunta" => array(), "shuntmv" => array(), "per" => array(), "watt_mode_enabled" => array() ,"watt_base_vm" => array(),"slope_enable" => array(),"limit_enable" => array(),"lower_limit" => array(),"upper_limit" => array() );
$vm_info['name'][] = "bump";
$vm_info['mode'][] = "bump";
$vm_info['shunta'][] = "bump";
...
$result  = $dbh->query('SELECT * FROM iso_voltmeters');
...
$result  = $dbh->query('SELECT * FROM iso_vm_graph_opts');
foreach($result as $row)
	{
...
function graph_overview()
	...
	$fakeh = $graph_height + 120;
		...
		echo "<div id='wrapper'>\n";
		echo "	<div class='row'>\n";
		echo "		<div class='col-sm-12' style='max-width:1100px'>\n";
  	echo "			<h3 style='text-align:center;'>USB Voltmeter Graphs - Data for the last $timespan - Click on a Graph to drill down.</h3>\n";
		echo "		</div>\n";
		echo "	</div>\n";
		...
		$filename = "/var/rrd/usb-vm1-" . $timespan . ".png";
		if (file_exists($filename)) 
		{
  	  //file exists
  	  echo "<a href='usb-vm-graph.php?action=viewgraph&g=vm1'><img src='/rrd/tmp/usb-vm1-" . $timespan . ".png' title='Click for more details'></a>   \n";
		} 
		else 
		{
  	  //file does not exist
		...
		echo "      	<div class='hpanel'>\n";
		echo "          <div class='panel-body' style='background:#F1F3F6;border:none;'>\n";
		$filename = "/var/rrd/usb-vm2-" . $timespan . ".png";
		if (file_exists($filename)) 
		{
  	  //file exists
  	  echo "<a href='usb-vm-graph.php?action=viewgraph&g=vm2'><img src='/rrd/tmp/usb-vm2-" . $timespan . ".png' height='170' title='Click for more details'></a>   \n";
		} 
		else 
		{
  	  //file does not exist
  	  echo "<img src='images/no-rrd-voltmeter.jpg' height='170' title='Data not Available'>  \n";
		}         	          		
		echo "        	 </div>\n";
		echo "        	</div>\n";
		echo "      	</div>\n";
		...
		echo "      	<div class='hpanel'>\n";
		echo "          <div class='panel-body' style='background:#F1F3F6;border:none;'>\n";	          	
		$filename = "/var/rrd/usb-vm3-" . $timespan . ".png";
		if (file_exists($filename)) 
		{
  	  //file exists
  	  echo "<a href='usb-vm-graph.php?action=viewgraph&g=vm3'><img src='/rrd/tmp/usb-vm3-" . $timespan . ".png' height='170' title='Click for more details'></a>   \n";
		} 
		else 
		{
  	  //file does not exist
		...
		echo "      	<div class='hpanel'>\n";
		echo "          <div class='panel-body' style='background:#F1F3F6;border:none;'>\n";
		$filename = "/var/rrd/usb-vm4-" . $timespan . ".png";
		if (file_exists($filename)) 
		{
  	  //file exists
  	  echo "<a href='usb-vm-graph.php?action=viewgraph&g=vm4'><img src='/rrd/tmp/usb-vm4-" . $timespan . ".png' height='170' title='Click for more details'></a>   \n";
		} 
		else 
		{
  	  //file does not exist
		...
		echo "      	<div class='hpanel'>\n";
		echo "          <div class='panel-body' style='background:#F1F3F6;border:none;'>\n";	          	
		$filename = "/var/rrd/usb-vm5-" . $timespan . ".png";
		if (file_exists($filename)) 
		{
  	  //file exists
  	  echo "<a href='usb-vm-graph.php?action=viewgraph&g=vm5'><img src='/rrd/tmp/usb-vm5-" . $timespan . ".png' height='170' title='Click for more details'></a>   \n";
		} 
		else 
		{
  	  //file does not exist
		...
		echo "      	<div class='hpanel'>\n";
		echo "          <div class='panel-body' style='background:#F1F3F6;border:none;'>\n";
		$filename = "/var/rrd/usb-vm6-" . $timespan . ".png";
		if (file_exists($filename)) 
		{
  	  //file exists
  	  echo "<a href='usb-vm-graph.php?action=viewgraph&g=vm6'><img src='/rrd/tmp/usb-vm6-" . $timespan . ".png' height='170' title='Click for more details'></a>   \n";
		} 
		else 
		{
  	  //file does not exist
...
function action_viewgraph()
	...
	if ($g == "vm1" || $g == "vm2" || $g == "vm3" || $g == "vm4" || $g == "vm5" || $g == "vm6")
		{
		$vm = substr($g, 2);
		echo "<!-- Main Wrapper -->\n";
			echo "<div id='wrapper'>\n";
			...
			echo "				<h3 style='text-align:center;'><u>USB Voltmeter " . $vm . " History - Click on a Graph to Zoom</u></h3>\n";
			echo "			</div>\n";
			echo "		</div>\n";
			echo "	</div>\n";
			if($screen_animations == "CHECKED")
			...
			vm_graph($vm, "-1h", "", "");
			echo "<p><a href='usb-vm-graph.php?action=zoomgraph&g=vm". $vm . "&t=hour&graph_start=" . ($time_now - 3600) . "&graph_end=" . $time_now . "'>";
			echo "<img src='/rrd/tmp/usb-vm" . $vm . "-hour.png' title='Click for more details'></a></p>\n";
			...
			vm_graph($vm, "-1d", "", "");
			echo "<p><a href='usb-vm-graph.php?action=zoomgraph&g=vm". $vm . "&t=day&graph_start=" . ($time_now - 86400) . "&graph_end=" . $time_now . "'>";
			echo "<img src='/rrd/tmp/usb-vm" . $vm . "-day.png' title='Click for more details'></a></p>\n";
			...
			vm_graph($vm, "-1w", "", "");			
			echo "<p><a href='usb-vm-graph.php?action=zoomgraph&g=vm". $vm . "&t=week&graph_start=" . ($time_now - 604800) . "&graph_end=" . $time_now . "'>";
			echo "<img src='/rrd/tmp/usb-vm" . $vm . "-week.png' title='Click for more details'></a></p>\n";
			...
			vm_graph($vm, "-1m", "", "");			
			echo "<p><a href='usb-vm-graph.php?action=zoomgraph&g=vm". $vm . "&t=month&graph_start=" . ($time_now - 2419200) . "&graph_end=" . $time_now . "'>";
			echo "<img src='/rrd/tmp/usb-vm" . $vm . "-month.png' title='Click for more details'></a></p>\n";
			...
			vm_graph($vm, "-1y", "", "");			
			echo "<p><a href='usb-vm-graph.php?action=zoomgraph&g=vm". $vm . "&t=year&graph_start=" . ($time_now - 31536000) . "&graph_end=" . $time_now . "'>";
			echo "<img src='/rrd/tmp/usb-vm" . $vm . "-year.png' title='Click for more details'></a></p>\n";
			...
			vm_graph($vm, "-5y", "", "");			
			echo "<p><a href='usb-vm-graph.php?action=zoomgraph&g=vm". $vm . "&t=5year&graph_start=" . ($time_now - 157680000) . "&graph_end=" . $time_now . "'>";
			echo "<img src='/rrd/tmp/usb-vm" . $vm . "-5year.png' title='Click for more details'></a></p>\n";
			...
			vm_graph($vm, "-10y", "", "");			
			echo "<p><a href='usb-vm-graph.php?action=zoomgraph&g=vm". $vm . "&t=10year&graph_start=" . ($time_now - 315360000) . "&graph_end=" . $time_now . "'>";
			echo "<img src='/rrd/tmp/usb-vm" . $vm . "-10year.png' title='Click for more details'></a></p>\n";
			...
			echo "		<div class='col-sm-12'>\n";
			echo "			<div class='alert alert-info' role='alert'>\n";
  		echo "				<p style='text-align:center'><a HREF='usb-vm-graph.php'>Click here to go back to previous page</a></p>\n";			
			echo "			</div>\n";
			echo "		</div>\n";
			echo "	</div>\n";
...
function action_zoomgraph()
	{
	global $tmp_path,$graph_width,$graph_height;
	
	
	$g = $_GET["g"];	// $g = vm1 vm2 temp load etc
	$t = $_GET["t"];	// $t = hour day week etc
	if($g=="vm1"){$gname = "USB Voltmeter 1";}
	else if($g=="vm2"){$gname = "USB Voltmeter 2";}
	else if($g=="vm3"){$gname = "USB Voltmeter 3";}
	else if($g=="vm4"){$gname = "USB Voltmeter 4";}
	else if($g=="vm5"){$gname = "USB Voltmeter 5";}
	else if($g=="vm6"){$gname = "USB Voltmeter 6";}
	...
	echo "		</script>\n";
	echo "<br><br><br><br>";		
  echo "<h3 style='text-align:center;'>USB Voltmeter " . $gname . " History - Drag Mouse on Graph to Zoom</h3>\n";
	...
	if ($g == "vm1" || $g == "vm2" || $g == "vm3" || $g == "vm4" || $g == "vm5" || $g == "vm6")
		...
		vm_graph( $vm, $_GET["graph_start"], $_GET["graph_end"], "$tmp_path/usb-vm" . $zoom_file);
		echo "<p style='text-align:center;'><img id='zoomGraphImage' src='/rrd/tmp/usb-vm" . $zoom_file . "?action=zoomgraph&g=" . $g . "&t=" . $t;
		}
	...
	print_zoom_js();
	echo"<p style='text-align:center;'><a href='usb-vm-graph.php?action=viewgraph&g=".$g."'>Click here to go back to the previous page</a></p>\n";
	html_footer();
	}
...
function vm_graph( $vmnum, $start, $end, $temp_filename )
	...
	if($start == "-1h")	
		{	
		$filename = "$tmp_path/usb-vm" . $vmnum . "-hour." . $img_type . "";
		$g_start_text = str_replace(":", '\:', date("Y-m-d H:i", ($time_now -  3600) ) );
		$timespan = " - (1 Hour)";	
		}
	else if($start == "-1d")	
		{	
		$filename = "$tmp_path/usb-vm" . $vmnum . "-day." . $img_type . ""; 
		$g_start_text = str_replace(":", '\:', date("Y-m-d H:i", ($time_now -  86400) ) );
		$timespan = " - (1 Day)";	
		}
	else if($start == "-1w")	
		{
		$filename = "$tmp_path/usb-vm" . $vmnum . "-week." . $img_type . "";
		$g_start_text = str_replace(":", '\:', date("Y-m-d H:i", ($time_now -  604800) ) );
		$timespan = " - (1 Week)";	
		}
	else if($start == "-1m")	
		{	
		$filename = "$tmp_path/usb-vm" . $vmnum . "-month." . $img_type . "";
		...
		{	
		$filename = "$tmp_path/usb-vm" . $vmnum . "-year." . $img_type . "";
		$g_start_text = str_replace(":", '\:', date("Y-m-d H:i", ($time_now -  31536000)  ) );
		$timespan = " - (1 Year)";	
		}
	else if($start == "-5y")	
		{	
		$filename = "$tmp_path/usb-vm" . $vmnum . "-5year." . $img_type . "";
		$g_start_text = str_replace(":", '\:', date("Y-m-d H:i", ($time_now -  157680000)  ) );
		$timespan = " - (5 Years)";	
		}
	else if($start == "-10y")	
		{	
		$filename = "$tmp_path/usb-vm" . $vmnum . "-10year." . $img_type . "";
		...
		if($vm_info['watt_mode_enabled'][$vmnum] == "CHECKED")
			{
				...
				$opts = array("--start", $start,
					...
					"COMMENT:\s",
					"--title=USB Wattmeter $vmnum - " . $vm_info['name'][$vmnum],
					"--vertical-label=Watts",
					"DEF:rawavg=$tmp_path/usbvb.rrd:vm" . $vmnum . ":AVERAGE",
					"DEF:baseavg=$tmp_path/usbvb.rrd:vm" . $vb . ":AVERAGE",
					...
					"GPRINT:ampavg:AVERAGE:Ave\:%8.3lf%s",
					"GPRINT:ampavg:LAST:Cur\:%8.3lf%s\\n",

//					"DEF:rawmin=$tmp_path/rms.rrd:vm" . $vmnum . ":MIN",
//					"DEF:basemin=$tmp_path/rms.rrd:vm" . $vb . ":MIN",
//					"CDEF:ampmin=rawavg,1000,*,$shunt_val,*,basemin,*",
//					"LINE1:ampmin" . $min_color . ":Minimum     ",
//					//"COMMENT:\n",
//					"GPRINT:ampmin:MIN:Min\:%3.2lf%s",
//					"GPRINT:ampmin:MAX:Max\:%3.2lf%s",
//					"GPRINT:ampmin:AVERAGE:Ave\:%3.2lf%s",
//					"GPRINT:ampmin:LAST:Cur\:%3.2lf%s",
//
//					"DEF:rawmax=$tmp_path/rms.rrd:vm" . $vmnum . ":MAX",
//					"DEF:basemax=$tmp_path/rms.rrd:vm" . $vb . ":MAX",
//					"CDEF:ampmax=rawavg,1000,*,$shunt_val,*,basemax,*",
//					"LINE1:ampmax" . $max_color . ":Maximum     ",
//					//"COMMENT:\n",
//					"GPRINT:ampmax:MIN:Min\:%3.2lf%s",
//					"GPRINT:ampmax:MAX:Max\:%3.2lf%s",
//					"GPRINT:ampmax:AVERAGE:Ave\:%3.2lf%s",
//					"GPRINT:ampmax:LAST:Cur\:%3.2lf%s",
					);
			}
		else
			{
				...
				$opts = array("--start", $start,
					...
					"COMMENT:FROM\: $g_start_text - TO\: $g_end_text" . $timespan . "\c",
					"COMMENT:\s",
					"--title=USB Ammeter $vmnum - " . $vm_info['name'][$vmnum],
					"--vertical-label=Amps",
					"DEF:rawavg=$tmp_path/usbvb.rrd:vm" . $vmnum . ":AVERAGE",
...
//					"DEF:rawmin=$tmp_path/rms.rrd:vm" . $vmnum . ":MIN",
//					"CDEF:ampmin=rawmin,1000,*,$shunt_val,*",
//					"LINE1:ampmin" . $min_color . ":Minimum     ",
//					//"COMMENT:\n",
//					"GPRINT:ampmin:MIN:Min\:%3.2lf%s",
//					"GPRINT:ampmin:MAX:Max\:%3.2lf%s",
...
//
//					"DEF:rawmax=$tmp_path/rms.rrd:vm" . $vmnum . ":MAX",
//					"CDEF:ampmax=rawmax,1000,*,$shunt_val,*",
//					"LINE1:ampmax" . $max_color . ":Maximum     ",
//					//"COMMENT:\n",
//					"GPRINT:ampmax:MIN:Min\:%3.2lf%s",
//					"GPRINT:ampmax:MAX:Max\:%3.2lf%s",
...
//					"GPRINT:ampmax:LAST:Cur\:%3.2lf%s",
		...
		$opts = array("--start", $start,
									...
									"COMMENT:FROM\: $g_start_text - TO\: $g_end_text" . $timespan . "\c",
									"COMMENT:\s",
									"--title=USB Voltmeter $vmnum - " . $vm_info['name'][$vmnum],
									"--vertical-label=Volts",
									"DEF:a=$tmp_path/usbvb.rrd:vm" . $vmnum . ":AVERAGE",
									"CDEF:grnpos=a,0,MAX",
									"CDEF:redneg=a,0,MIN",
									...
									"GPRINT:a:AVERAGE:Ave\:%8.3lf%s",
									"GPRINT:a:LAST:Cur\:%8.3lf%s\\n",

//									"DEF:rawmin=$tmp_path/rms.rrd:vm" . $vmnum . ":MIN",
//									"LINE1:rawmin" . $min_color . ":Minimum",
//									//"COMMENT:\n",
//									"GPRINT:rawmin:MIN:Min\:%8.3lf%s",
//									"GPRINT:rawmin:MAX:Max\:%8.3lf%s",
//									"GPRINT:rawmin:AVERAGE:Ave\:%8.3lf%s",
//									"GPRINT:rawmin:LAST:Cur\:%8.3lf%s\\n",
//
//									"DEF:rawmax=$tmp_path/rms.rrd:vm" . $vmnum . ":MAX",
//									"LINE1:rawmax" . $max_color . ":Maximum",
//									//"COMMENT:\n",
//									"GPRINT:rawmax:MIN:Min\:%8.3lf%s",
//									"GPRINT:rawmax:MAX:Max\:%8.3lf%s",
//									"GPRINT:rawmax:AVERAGE:Ave\:%8.3lf%s",
//									"GPRINT:rawmax:LAST:Cur\:%8.3lf%s\\n",
...
function html_header()
    ...
    echo "		<!-- Page title -->\n";
    echo "		<title>USB Voltmeter Graphs</title>\n";
    $myrand = rand();
...
function action_gen_graphs()
	...
	for ( $i=1; $i<=6; $i++)	{ vm_graph($i, $start, "", ""); }
	clean_temp_images();
	}
...
// Global constant
var cURLBase = "usb-vm-graph.php?action=zoomgraph";
